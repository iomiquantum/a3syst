...
serve(async (req) => {
  ...
  try {
    ...
    const { createClient: createAuthClient } = await import("https://esm.sh/@supabase/supabase-js@2.95.3");
    const authClient = createAuthClient(SUPABASE_URL, Deno.env.get("SUPABASE_ANON_KEY")!, {
